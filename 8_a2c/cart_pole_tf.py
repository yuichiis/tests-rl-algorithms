...
ENTROPY_COEFF = 0.01
NUM_ITERATIONS, STEPS_PER_ITERATION, MAX_STEPS_PER_EPISODE = 400, 256, 600

# 環境とエージェントの初期化
cart_pole = gym.make("CartPole-v1")
agent = A2CAgent(cart_pole.observation_space.shape[0], cart_pole.action_space.n, ACTOR_LR, CRITIC_LR, GAMMA, GAE_LAMBDA, ENTROPY_COEFF)
...
state, _ = cart_pole.reset() # stateはtf.Tensor
...
for iteration in range(NUM_ITERATIONS):
    ...
    for _ in range(STEPS_PER_ITERATION):
        ...
        next_state, reward, done, truncated, _ = cart_pole.step(action) # next_stateもtf.Tensor
