...
# ==============================================================================
# 1. Gymnasium互換の迷路環境 (MazeEnv) の定義 (★★★★★ モダン化 ★★★★★)
# ==============================================================================
class MazeEnv(gym.Env):
    ...
    def step(self, action: int) -> Tuple[Dict, float, bool, bool, Dict]:
        # action_maskにより不正な行動はエージェント側で選択されないと仮定
...
env = MazeEnv(maze_map=maze_map_3x3)
    ...
    for r in range(height):
        for c in range(width):
            ...
            if (r, c) == goal_pos:
                ...
                with torch.no_grad():
                    action_probs = policy_net(state_tensor, mask=action_mask_tensor)
